echo 现在开始Windows安全加固，确认请按任意键 by githup/dashuaib
pause
...
echo LockoutBadCount=5 >>account.inf REM 设定帐户锁定阀值为5次
...
echo AuditLogRetentionPeriod=0 >>logcfg.inf REM 设置当达到最大的日志尺寸时按需要改写事件
echo RestrictGuestAccess=1 >>logcfg.inf REM 设置限制GUEST访问应用日志
echo [Security Log] >>logcfg.inf REM 设置安全日志
echo MaximumLogSize=8192 >>logcfg.inf REM 设置安全日志文件最大8192KB
echo AuditLogRetentionPeriod=0 >>logcfg.inf REM 设置当达到最大的日志尺寸时按需要改写事件
echo RestrictGuestAccess=1 >>logcfg.inf REM 设置限制GUEST访问安全日志
echo [Application Log] >>logcfg.inf REM 设置应用日志
echo MaximumLogSize=8192 >>logcfg.inf REM 设置安全日志文件最大8192KB
echo AuditLogRetentionPeriod=0 >>logcfg.inf REM 设置当达到最大的日志尺寸时按需要改写事件
echo RestrictGuestAccess=1 >>logcfg.inf REM 设置限制GUEST访问安全日志
...
echo="正在为您添加危险端口阻止组策略"
netsh ipsec static add policy name="ONION安全策略" description="阻止onion访问指定的端口"
netsh ipsec static add filteraction name="阻止" action=block

netsh ipsec static add filterlist name="阻止访问445等"
netsh ipsec static add filter filterlist="阻止访问445等" srcaddr=any dstaddr=me dstport=135 protocol=tcp mirrored=yes
netsh ipsec static add filter filterlist="阻止访问445等" srcaddr=any dstaddr=me dstport=135 protocol=udp mirrored=yes

netsh ipsec static add filter filterlist="阻止访问445等" srcaddr=any dstaddr=me dstport=137 protocol=tcp mirrored=yes
netsh ipsec static add filter filterlist="阻止访问445等" srcaddr=any dstaddr=me dstport=137 protocol=udp mirrored=yes

netsh ipsec static add filter filterlist="阻止访问445等" srcaddr=any dstaddr=me dstport=138 protocol=tcp mirrored=yes
netsh ipsec static add filter filterlist="阻止访问445等" srcaddr=any dstaddr=me dstport=138 protocol=udp mirrored=yes

netsh ipsec static add filter filterlist="阻止访问445等" srcaddr=any dstaddr=me dstport=139 protocol=tcp mirrored=yes
netsh ipsec static add filter filterlist="阻止访问445等" srcaddr=any dstaddr=me dstport=139 protocol=udp mirrored=yes

netsh ipsec static add filter filterlist="阻止访问445等" srcaddr=any dstaddr=me dstport=445 protocol=tcp mirrored=yes
netsh ipsec static add filter filterlist="阻止访问445等" srcaddr=any dstaddr=me dstport=445 protocol=udp mirrored=yes

netsh ipsec static add rule name="禁止访问的终端策略规则" policy="ONION安全策略" filterlist="阻止访问445等" filteraction="阻止"
netsh ipsec static set policy name="ONION安全策略" assign = y

echo="已完成添加危险端口阻止组策略  by githup/dashuaib"
pause
